...
def create_app(project_dir: Path):
    ...
    def filter_tasks(search: str = "", status: list[str] | None = None, agent: list[str] | None = None) -> list[dict]:
        """Filter tasks by search query, status, and assigned agent."""
        tasks = list_tasks(db())
    ...
    def get_filtered_tasks() -> str:
        """GET /tasks/filtered - Get filtered tasks based on query parameters."""
        search = request.args.get("search", "").strip()
        status_list = request.args.getlist("status")
        agent_list = request.args.getlist("agent")

        # Get filtered tasks
        filtered = filter_tasks(search, status_list or None, agent_list or None)
