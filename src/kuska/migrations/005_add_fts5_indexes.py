...
def up(migrator, db):
    ...
    try:
        with db.atomic():
            ...
            # Create triggers to keep docs_fts in sync
            db.execute_sql("""
            ...
            db.execute_sql("""
                ...
                AFTER UPDATE ON docs BEGIN
                    UPDATE docs_fts SET key = new.key, content = new.content
            ...
            db.execute_sql("""
                ...
                AFTER UPDATE ON messages BEGIN
                    UPDATE messages_fts
                    SET payload = new.payload, msg_type = new.msg_type, sender = new.sender
                    WHERE rowid = old.id;
                END
            """)
            ...
            db.execute_sql("""
                ...
                AFTER UPDATE ON events BEGIN
                    UPDATE events_fts SET body = new.body, label = new.label, kind = new.kind
                    WHERE rowid = old.id;
                END
            """)
